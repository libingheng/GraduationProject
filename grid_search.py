    ...
    def main(self):
        ...
        if self.model_name == 'textcnn':
            trainer = TextCNNTrainer(emb_dim=self.emb_dim, mlp_dims=self.mlp_dims,
                                     ...
                                     dropout=self.dropout, weight_decay=self.weight_decay, val_loader=val_loader,
                                     test_loader=test_loader, category_dict=self.category_dict,
                                     early_stop=self.early_stop, epoches=self.epoch, dataset=self.dataset,
            ...
            trainer = BiGRUTrainer(emb_dim=self.emb_dim, mlp_dims=self.mlp_dims, dataset=self.dataset,
                                   use_cuda=self.use_cuda, lr=self.lr, train_loader=train_loader, dropout=self.dropout,
                                   weight_decay=self.weight_decay, val_loader=val_loader, test_loader=test_loader,
                                   category_dict=self.category_dict, num_layers=1, early_stop=self.early_stop,
                                   epoches=self.epoch,
                                   save_param_dir=os.path.join(self.save_param_dir, self.model_name))
            ...
            trainer = M3FENDTrainer(emb_dim=self.emb_dim, mlp_dims=self.mlp_dims, use_cuda=self.use_cuda, lr=self.lr,
                                    train_loader=train_loader, dropout=self.dropout, weight_decay=self.weight_decay,
                                    ...
                                    save_param_dir=os.path.join(self.save_param_dir, self.model_name),
                                    semantic_num=self.semantic_num, emotion_num=self.emotion_num,
                                    style_num=self.style_num, lnn_dim=self.lnn_dim, dataset=self.dataset)
